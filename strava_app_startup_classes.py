	...
	def __init__(self, master, bg, width, height, x, y, text, fg, font, dropdown_button_list):
		...
		for index, button in enumerate(dropdown_button_list, 2):
			self.dropdown_buttons.append(
				Buttons(self, button[0], bg, fg, font, button[1], self.width, 25, 0, 25 * int(index)))
...
def registration_screen():
	...
	button_auth_page = Buttons(frame_auth, "Authorization page", "white", "black", "Trebuchet", open_auth_page, 300, 25,
							   12.5,
							   50)
	...
	entry_password_confirm = Entries(frame_account, confirm_password, "Trebuchet 12", 300, 25, 12.5, 125, "*",
									 "disabled")
	button_create_account = Buttons(root_regis, "Create account", "white", "black", "Trebuchet 15", add_account, 300,
									50,
									25, 363)
